...
void EmojiCollection::LoadDefaultEmoji(const std::string& path) {
    ESP_LOGI(TAG, "Loading default emojis from SD card path: %s", path.c_str());
    // Current implementation of GetEmojiImage already checks SD card.
    // This method can be expanded if needed to pre-scan the directory.
}
